...
vector <int> digits;
ll pow10[20];
pair <ll,ll> dp[20][1<<10][2][2];
bool vis[20][1<<10][2][2];
int K,N;
pair<ll,ll> dfs(int pos,int mask,int tight,int lz)
{
	if(pos==N)
		return {1,0};
	auto &res=dp[pos][mask][tight][lz];
	if(vis[pos][mask][tight][lz])
		return res;
...
int main()
{
	...
	ll ans=(solveUpTo(r)-solveUpTo(l-1))%MOD;
	if(ans<0)
		ans+=MOD;
	cout<<ans<<endl;
	return 0;
